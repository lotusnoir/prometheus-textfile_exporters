#!/usr/bin/env bash
#===============================================================================
#         FILE:  freeipa_exporter.sh
#
#        USAGE:  ./freeipa_exporter.sh > /var/lib/node_exporter/freeipa.prom
#                (or via cron/systemd timer, redirected atomically by the caller)
#
#  DESCRIPTION:
#                Collects FreeIPA server health, replication topology,
#                certificate expiry, LDAP/SNMP counters, Kerberos, HTTPD and
#                SSSD status, and exposes them as Prometheus metrics.
#                Metrics are PRINTED TO STDOUT ONLY: the caller is responsible
#                for the atomic write into the node_exporter textfile
#                collector directory (e.g. `... > out.prom.$$ && mv out.prom.$$ out.prom`).
#
#  EXIT CODE:    0 = OK   (metrics collected, IPA server responded to ping)
#                1 = FAIL (IPA server did not respond to ping / hard failure)
#                Note: this exit code convention (0=ok, 1=not ok) is the
#                standard Unix one and is independent from individual metric
#                values (e.g. freeipa_server_up uses 1=up/0=down, see HELP).
#
#  REQUIREMENTS: bash 4+, ipa/ipactl/ipa-healthcheck, ldapsearch, klist,
#                getcert, systemctl, sssctl (all optional; missing tools are
#                skipped gracefully)
#       AUTHOR:  Philippe LEAL
#      VERSION:  2.0
#      CREATED: 2026-09-18
#      UPDATED: 2026-09-23 (stdout output instead of file, exit code convention)
#===============================================================================
...
# Function to safely write metrics (stdout)
write_metric() {
    local metric_name="$1"
...
# 9. Basic LDAP monitor/SNMP data
if command -v ldapsearch &>/dev/null && [ -n "$LDAP_SOCKET" ]; then
    ...
    if [ -n "$LDAP_VERSION" ]; then
        write_metric_header "freeipa_ldap_info" "LDAP server information" "gauge"
        write_metric_with_labels "freeipa_ldap_info" "1" "version=\"${LDAP_VERSION}\""
    fi
...
if command -v klist &>/dev/null; then
    ...
    if [ -n "$DEFAULT_PRINCIPAL" ]; then
        write_metric_header "freeipa_krb5_default_principal" "Default Kerberos principal" "gauge"
        write_metric_with_labels "freeipa_krb5_default_principal" "1" "principal=\"${DEFAULT_PRINCIPAL}\""
    fi
fi
...
# Exit code convention: 0 = OK, 1 = NOT OK (based on IPA server reachability)
if [ "${IPA_STATUS}" -eq 1 ]; then
    exit 0
else
    exit 1
fi
